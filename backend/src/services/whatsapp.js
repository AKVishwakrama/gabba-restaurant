...
function formatBillMessage(order, items, recipientLabel) {
  const lines = items
    .map((it) => `- ${it.item_name} x${it.quantity} • ₹${Number(it.price).toFixed(2)} each • ₹${(Number(it.price) * Number(it.quantity)).toFixed(2)}`)
    .join("\n");
  ...
  const summary = [
    ...
    "",
    `Payment: ${String(order.payment_method || "cod").toUpperCase()} (${order.payment_status || "pending"})`,
...
async function sendWhatsAppMessage(toNumber, body) {
  const c = getClient();
  if (!c) {
    console.warn("[whatsapp] Twilio not configured - skipping send. Set TWILIO_* env vars.");
    return { skipped: true };
  }
  try {
...
async function sendBillToCustomerAndOwner(order, items) {
  const customerMsg = formatBillMessage(order, items, "customer");
  const ownerMsg = formatBillMessage(order, items, "owner");

  const results = { customer: null, owner: null };

  if (order.customer_phone) {
    results.customer = await sendWhatsAppMessage(order.customer_phone, customerMsg);
  }
  if (process.env.OWNER_WHATSAPP_NUMBER) {
    results.owner = await sendWhatsAppMessage(process.env.OWNER_WHATSAPP_NUMBER, ownerMsg);
  }
  return results;
}

module.exports = { sendBillToCustomerAndOwner, sendWhatsAppMessage, formatBillMessage };
